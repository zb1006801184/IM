...
//                    [audio playSongWithUrl:voiceURL];
    ...
    else if (self.messageFrame.message.type == UUMessageTypePicture)
    {
        if (self.btnContent.backImageView) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(cellContentDidClick:image:)]) {
                [self.delegate cellContentDidClick:self image:self.btnContent.backImageView.image];
            }
        }
