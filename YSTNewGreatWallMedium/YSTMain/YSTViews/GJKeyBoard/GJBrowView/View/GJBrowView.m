...
@interface GJBrowView ()
//{
//    ELBrowShowV *_browShowV;
//    ELBrowRaiseV *_browRaiseV;
//}

@property (nonatomic, strong) GJBrowShowV *browShowV;
@property (nonatomic, strong) GJBrowRaiseV *browRaiseV;
@property (nonatomic, copy) NSMutableArray *emoArray;

@end

@implementation GJBrowView
...
- (void)loadData
{
    ...
    //第一组系统表情
    NSMutableDictionary *sysEmoDic = [GJEmoManage emotionManage:@"face.plist" plistType:NSDictionaryType itemCount:24];

    //第二组，系统的小表情
    NSMutableDictionary *sysSmaEmoDic = [GJEmoManage emotionManage:@"Emoji.plist" plistType:NSArrayType itemCount:24];
    
    //第三组，添加的表情
    
    NSMutableDictionary *addEmoDic = [GJEmoManage emotionManage:@"emotion.plist" plistType:NSDictionaryType itemCount:8];
...
{
    //显示表情的view
    _browShowV = [[GJBrowShowV alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height-30) withEmoArray:self.emoArray];
//    browShowV.emoArray = self.emoArray;
    [self addSubview:_browShowV];
    
    //下方控制切换表情的view，添加表情，设置等
    _browRaiseV = [[GJBrowRaiseV alloc] initWithFrame:CGRectMake(0, _browShowV.bottom, self.width, self.height-_browShowV.height)];
