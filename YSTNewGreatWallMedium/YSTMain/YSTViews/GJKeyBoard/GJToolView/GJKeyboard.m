...
#define GJScreenHeight [[UIScreen mainScreen] bounds].size.height
#define GJScreenWidth  [[UIScreen mainScreen] bounds].size.width

@interface GJKeyboard ()<GJKeyboardCenterDelegate, GJTextViewDelegate>

@property (nonatomic, assign) CGFloat initHeight; //初始化 GJKeyboard 的高度
...
//    NSLog(@"-------bottom   %f", self.bottom);
    // 设置代理
    [GJKeyboardCenter defaultCenter].delegate = self;
    ...
    //表情
    _browView = [[GJBrowView alloc] initWithFrame:CGRectMake(0, 0, self.width, KeyboardHeight)];
    //更多
    _moreView = [[GJMoreView alloc] initWithFrame:CGRectMake(0, 0, self.width, KeyboardHeight)];

    
    //输入框
    _textView = [[GJTextView alloc] initWithFrame:CGRectMake(_voiceBtn.right+textVItl, (self.height-textVHight)/2, self.width-interval-2*textVItl-3*KeyBoardBtnWidth-interval, textVHight)];
    ...
    _textView.changeHeightBlock = ^(CGFloat height) {
        ...
        weakself.frame = CGRectMake(0, GJScreenHeight-(height+(weakself.initHeight-textVHight))-weakself.oneselfHeight, weakself.width, height+(weakself.initHeight-textVHight));
...
//        NSLog(@"weakself.textView.frame   %@", weakself.textView);
    ...
    _tvVoiceBtn = [[GJTVVoiceBtn alloc] initWithFrame:_textView.bounds];
...
- (void)voiceSceneAction:(UIButton *)button
{
    ...
    if (button.selected) {
        ...
        self.frame = CGRectMake(0, GJScreenHeight-(self.textViewHeight+14)-self.oneselfHeight, self.width, self.textViewHeight+14);
        ...
        //设置frame 最开始的frame
        self.frame = CGRectMake(0, GJScreenHeight-_initHeight-self.oneselfHeight, self.width, _initHeight);
...
//    if ([self.delegate respondsToSelector:@selector(sendMessageTextheight:)]) {
//        [self.delegate sendMessageTextheight:_oneselfHeight];
//    }
}
...
#pragma mark - GJKeyboardCenterDelegate
...
//    NSLog(@"ViewController1 接收到%@通知\n高度值：%f\n时间：%f",isShow ? @"弹出":@"隐藏", height,animationDuration);
    ...
    self.top = GJScreenHeight - height - self.height;
    ...
    _timer = nil;
    double time = .1;
    _timer = [NSTimer scheduledTimerWithTimeInterval:time target:self selector:@selector(delayMethod) userInfo:nil repeats:NO];
