...
- (void)addSpecifiedItem:(NSDictionary *)dic
{
    ...
    if ([dic[@"types"] isEqualToString:@"2"]) {
        [dataDic setObject:@(UUMessageFromOther) forKey:@"from"];
        [dataDic setObject:dic[@"nickName"] forKey:@"strName"];
    }else{
        [dataDic setObject:@(UUMessageFromMe) forKey:@"from"];
        [dataDic setObject:[UserModel getModel].nickName forKey:@"strName"];
    }
    [dataDic setObject:[self getCurrentTimes] forKey:@"strTime"];
//    NSLog(@"%@",dataDic[@"strTime"]);
...
- (NSString *)getImageStr:(NSInteger)index{
    NSArray *array = @[@"http://www.120ask.com/static/upload/clinic/article/org/201311/201311061651418413.jpg",
                       ...
                       @"http://ts1.mm.bing.net/th?&id=JN.7g7SEYKxd2MTNono6zVirpA&w=300&h=300&c=0&pid=1.9&rs=0&p=0"];
    return array[index];
}
